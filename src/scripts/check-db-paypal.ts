...
export default async function checkDbPaypal({ container }: { container: MedusaContainer }) {
  ...
  try {
    const result = await pgConnection.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE '%paypal%' ORDER BY table_name");
  ...
  try {
    const result = await pgConnection.execute("SELECT id, environment, is_active, created_at FROM paypal_credential LIMIT 5");
  ...
  try {
    const result = await pgConnection.execute("SELECT id, environment, data FROM paypal_onboarding LIMIT 5");
  ...
  try {
    const result = await pgConnection.execute("SELECT id, data FROM paypal_setting LIMIT 5");
