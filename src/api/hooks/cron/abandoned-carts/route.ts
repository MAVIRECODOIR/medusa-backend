...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  const logger = req.scope.resolve<{ info: (msg: string) => void; error: (msg: string) => void }>(ContainerRegistrationKeys.LOGGER)
  ...
  try {
    ...
    if (!carts?.length) {
      return res.json({ ok: true, sent: 0, reason: "no carts with email" })
    }
    ...
    for (const cart of carts as any[]) {
      ...
      sent++
      logger.info(`[cron-abandoned-cart] Sent for ${cart.email} (cart ${cart.id})`)
    }

    return res.json({ ok: true, sent })
  } catch (err: any) {
    logger.error(`[cron-abandoned-cart] Failed: ${err.message}`)
